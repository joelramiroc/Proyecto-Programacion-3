...
WithMovimentsHard::WithMovimentsHard(map<string,sf::Drawable*>& drawables, Cube* &cube,Window& window_,string nombrePlayer):window_(window_)
{
    ...
    this->Dispon=100;
    this->contara=Dispon;
    this->typeGame="WithMovimentsHard";

    mostrarNivel.setString("Dificil");
...
void WithMovimentsHard::comparations()
{
...
void WithMovimentsHard::info()
{
    Dispon--;
    cantidadRestante = "Disponibles: " + to_string(Dispon);
};

void WithMovimentsHard::events()
{
...
void WithMovimentsHard::cleared()
{
    window_.close();
}

void WithMovimentsHard::loop()
{
    scramble();
    while(repetir)
    {
